...
public class ProjectileThrower : MonoBehaviour {
    ...
    {
		Rigidbody2D[] list = FindObjectsOfType(typeof(Rigidbody2D)) as Rigidbody2D[];
		foreach (Rigidbody2D element in list) {
    ...
    internal void Update()
    {
        ...
        if(WaitingForPhysicsToSettle () != true && firingState == FiringState.Firing)
			ResetForFiring();
